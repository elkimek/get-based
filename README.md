...
- Official Vercel endpoints for encrypted profile sharing and public deployment metadata. A separately deployed, narrowly scoped compatibility relay handles supported wearable providers, NVIDIA attestation, the fixed privacy-rounded CAMS lookup, and credential-free public-page imports. Generic AI, voice, and custom-provider forwarding is rejected on getbased-operated hosts. Self-host-only providers use the deployment owner's OAuth credentials and, only where required, its same-origin proxy.
