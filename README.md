...
- **Connect wearables and body metrics** — first-class direct connections for Oura, Withings, Polar, plus WHOOP and Ultrahuman where enabled; Google Health for Fitbit, Pixel Watch, and optional aggregation; and local Apple Health file import. Existing legacy Fitbit connections remain only as a migration bridge through September 2026. Manual weight, blood pressure, and resting pulse work without a wearable.
...
- **Optional wearable clouds.** Independent direct providers and Google Health are separate opt-in connections. Google Health never silently backs up a direct connection. Its OAuth tokens and imported daily rows are always encrypted with a device-local key and excluded from raw-data backups, while token refreshes and Google API requests transit the deployment's Vercel proxy. Each browser must be connected separately; only the compact derived summary participates in optional profile backup/sync or user-enabled AI/agent context.
