...
- **Automatic** reuses a connected OpenRouter, PPQ, or Venice account when that provider supports the requested audio operation, and otherwise falls back to the browser-local models. A selected CLI agent receives only the transcribed text, not the recording.
