...
- **Connect wearables and body metrics** — first-class direct connections for Oura, Withings, Polar, plus WHOOP and Ultrahuman where enabled; local Apple Health file import; and manual weight, blood pressure, and resting pulse. Google Health support for Fitbit, Pixel Watch, and optional aggregation is prepared for self-hosted deployments using their own Google Cloud OAuth project, but is not enabled on the official hosted app. Existing legacy Fitbit connections remain only as a migration bridge through September 2026.
