...
The full CI browser run exposed seven outdated fixtures (provider count, isolated API exports, revised Genome copy, lazy mobile styles, and tour setup) and a cold-load budget overrun. Focused browser checks retain the original behaviors and now exercise the current contracts. The source-mode cold load measures 324 requests, 1,183,242 compressed bytes, and 3,319,406 decoded bytes. Its seven new modules are dashboard-widget-copy, chat-backend-selection, ai-feature-routing, agent-chat-settings, agent-model-catalog, ai-execution-routing, and reasoning-capabilities (34,498 decoded bytes combined). These provide the new synchronous provider/capability checks; changed existing resources account for the remaining increase. The source-mode request/decoded budgets explicitly allow this measured feature cost (324 requests / 3,320,000 bytes); the compressed-transfer ceiling remains 1,193,220 bytes. This is separate from the two-file production startup budget above. CI now retains the per-resource measurement as a test attachment so future regressions can be attributed rather than blindly increasing budgets.
