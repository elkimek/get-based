...
export function computeIronHandling(data, def, options = {}) {
  ...
  if (ferritin) {
    ...
    missing.push({ key: 'ferritin', label: 'Ferritin', weight: 1.15, core: true });
  }
  if (tsat) {
  ...
  add(hgb, 'hgb', 'Hemoglobin', 0.8, hgb ? scoreAgainstRange(hgb.value, hgb.range) : null, true);
  add(mch, 'mch', 'MCH', 0.65, mch ? scoreAgainstRange(mch.value, mch.range) : null);
  add(mcv, 'mcv', 'MCV red-cell size', 0.5, mcv ? scoreAgainstRange(mcv.value, mcv.range) : null);
  add(iron, 'iron', 'Serum iron', 0.55, iron ? scoreAgainstRange(iron.value, iron.range) : null);
  add(transferrin, 'transferrin', 'Transferrin', 0.45, transferrin ? scoreAgainstRange(transferrin.value, transferrin.range) : null);
  add(tibc, 'tibc', 'TIBC', 0.35, tibc ? scoreAgainstRange(tibc.value, tibc.range) : null);
  add(sTfR, 'sTfR', 'Soluble transferrin receptor', 0.55, sTfR ? scoreAgainstRange(sTfR.value, sTfR.range) : null);
  add(crp, 'crp', 'CRP', 0.65, crp ? scoreHighOnly(crp.value, crp.range?.max ?? 3, (crp.range?.max ?? 3) * 4) : null);
