...
export function e2eeLockHTML(attestation) {
  ...
  const mark = ok ? '\u2713' : '\u2717';
  return ` <span title="${attestationTooltip(attestation)}">\uD83D\uDD12<span style="color:${color};font-weight:bold">${mark}</span></span>`;
}
...
export function e2eeLockFootnote(attestation) {
  ...
  const color = ok ? '#22c55e' : '#ef4444';
  const mark = ok ? '\u2713' : '\u2717';
  return ` \u00b7 <span title="${attestationTooltip(attestation)}">\uD83D\uDD12<span style="color:${color};font-weight:bold">${mark}</span> e2ee</span>`;
}
