...
const CHANGELOG = [
  {
    ...
    items: [
      '<b>Sun isn\'t just vitamin D.</b> Different parts of sunlight do different things — set your body clock, support circulation, charge your mitochondria, regulate mood-hormones. The new <b>☀ Light & Sun</b> lens tracks your exposure across six biological channels (Vitamin D, Mood & hormones, Cardiovascular, Outdoor eye light, Body clock, Cellular repair) and lets you correlate them with your labs and wearable data over time.',
      '<b>One-tap sun session logging.</b> Tap when you go outside, tap again when you come back. We pull the actual UV + ozone + atmosphere for your location, reconstruct the solar spectrum at your zenith, and compute per-channel doses on the spot. Plain-English summary on stop: <i>"Saved · 22 min outside · ~600–1500 IU vitamin D · burn dose 35% — well within safe range."</i> Active session is pinned to the top of the page with a Live badge above the Stop button.',
      '<b>Sun-safety guardrails.</b> Live toast at 70% of your burn dose, hard alert at 100%. Photosensitizing-medication checkbox drops your threshold 2.5× (tetracyclines, doxycycline, isotretinoin, amiodarone, thiazides, sulfa antibiotics, NSAIDs, St. John\'s Wort, others). Carry-over chip warns when yesterday + today combined exceeds your daily limit. High-altitude UV chip flags >1500m locations. Eyes-uncovered mode triggers a clear "never look directly at the sun" reminder. First-time MED + ICNIRP toasts open with a one-line definition so the jargon teaches itself.',
      '<b>Light therapy devices, first-class.</b> Joovv panels, Mito Red, Sperti UVB, Verilux SAD, dawn simulators, full-spectrum bulbs — pick from a 24-device preset library or add a custom device. Therapy sessions feed the same channels as outdoor sun. When a device-fillable channel (660 nm red, 810/850 nm IR) stays empty over 30 days, the page surfaces a "Fill the {channel} with a device" card from the affiliate catalog — region-filtered, gated by Settings → Privacy.',
      '<b>Indoor light environment matters too.</b> Map the rooms you spend time in (LED type, hours/day, after-sunset use) and the screens you stare at. Each Ott audit question carries a one-line photobiology explainer below the prompt ("Window glass blocks UVB almost entirely — no vitamin D, no nitric-oxide release through the skin"). Deficit signals (LED-only contamination, blue-after-sunset hours) flow into the same channels.',
      '<b>Eight on-device measurement tools.</b> Lux Meter · Flicker Detector (PWM banding via 240 fps camera + FFT) · Color Temperature meter · Light classifier (LED / fluorescent / incandescent / daylight) · Glass transmission test · Sleep darkness meter (long-exposure check at the pillow) · Sunrise/sunset session logger · Eye-Level Audit walkthrough. Camera frames never leave your device.',
      '<b>Vit-D model recalibrated against published references.</b> Body-fraction presets capped at single-position max (Sunbathing = 0.50, not 0.90 — you can\'t have both sides exposed at once). New 🔄 Flip mid-session button doubles vitamin D yield to acknowledge fresh skin keeps synthesizing. IU constant bumped 40 → 60 to match dminder + NIWA at UVI 5–7. Eye-UV math gates on sun elevation ≥ 5° — no more phantom retinal-UV accumulation before sunrise. Diffuse-light component now scales with airMass, fixing the 30–50% UVB underestimate at extreme zenith (high-latitude winter, late-afternoon sun).',
      '<b>CAMS atmosphere relay live.</b> The new <code>getbased-uvdata</code> companion repo ships KNMI-validated total column ozone, aerosol optical depth, and PM2.5 / PM10 from the same satellite-assimilated source Open-Meteo wraps for AQI. The Sun data source picker defaults to "Best accuracy" — CAMS for atmospheric composition + Open-Meteo for clouds and temperature, automatically merged. UV source confidence is now computed (snapshot age, cloud cover, sun elevation, UVI band, stale-grid flag) instead of hardcoded — a reading at sunset under heavy cloud honestly drops to ~40%. 5-day forecast horizon.',
      '<b>For biohackers: depth without lock-in.</b> Manual UVI override feeds your own meter into the math. Each channel pill expands to its action spectrum + 2–3 PubMed/journal references (CIE 174:2006 for vit D, CIE S 026 for melanopic) and a 7-day rhythm chart. Saved sessions show solar zenith, altitude, ozone column, UVB/UVA split. Standard-tier chat auto-escalates on intent — ask anything sun/light/sleep/vit-D/circadian and your context expands to include the last 30 sessions plus computed biomarker correlations. New <code>getSunSessionsSlice()</code> + <code>getSunSessionDetail()</code> agent-callable APIs for MCP and self-hosted lenses.',
      ...
      '<b>AI sees the full picture.</b> Every chat now carries a Light & Sun summary — your active deficits, your device library, your week\'s per-channel exposure, your daily burn-dose state. A calibration anchor surfaces your last 25-OH-D bloodwork next to your modeled vit-D estimate so the AI can sanity-check itself. Once you have ≥4 weeks of overlapping sessions and labs, channel-by-biomarker correlations join the standard tier. Deficit detection is gated behind ≥7 logged events so brand-new users don\'t see false "no exposure logged" claims.',
      '<b>Sync got a structural upgrade.</b> Cross-device sync now ships per-row CRDT deltas instead of one fat blob — every push carries only what changed. Concurrent edits from phone and desktop merge cleanly per-row across markerNotes, changeHistory, customMarkers, manualValues, sun sessions, light environment, and chat summaries. Each push is gzipped on the wire. Result: faster pushes, less relay traffic, and no more phone-edit-clobbers-desktop-edit when both happen in the same window.',
      '<b>Self-serve relay storage.</b> When you hit the per-account relay storage cap, the Sync diagnose modal now has a Compact storage button that drops the older Evolu message log directly — no maintainer round-trip, no SSH access required. A Refresh button shows the relay\'s real <code>storedBytes</code> instead of a drifting local estimate. Both endpoints HMAC-SHA256-signed with your own writeKey; one user can never act on another user\'s owner. Per-IP rate limits (10 compact/min) with 429 + Retry-After. Storage indicator toasts at 80% (amber) and 95% (red) before you hit the wall.',
      '<b>Privacy posture.</b> Location resolves from your country (set in profile) — no automatic geolocation prompt at session start. Opt-in to precise location once for sharper estimates. UV/ozone fetches via Open-Meteo by default; self-host the CAMS relay via the External-server panel with HTTPS-required hardening (closes a DNS-rebinding gap). Sun Data Source picker (auto / self-host URL+bearer / manual) and privacy-rounding toggle live in Settings → Privacy → Sun. Camera frames and sensor readings stay on-device.',
      ...
      '<b>Behind the scenes.</b> Vitamin D math, IndexedDB storage for large blobs, accessibility (keyboard + screen-reader), gzip-bomb defence on every decompression path, snapshot-poisoning fix on partial push failures, DST-safe peak finder, owner-scoped quota markers, prototype-pollution defence on per-row pulls — all hardened in this release.',
    ]
  },
  {
