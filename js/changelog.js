...
const CHANGELOG = [
  {
    ...
    items: [
      '<b>Three strong-evidence oxidative-stress SNPs added to the genetics section</b> in a new "Oxidative Stress" category: <code>NQO1 Pro187Ser</code> (rs1800566 — variant protein is rapidly proteasomally degraded, half-life ~1.2h vs &gt;18h wild-type, Siegel 2001), <code>PON1 Q192R</code> (rs662 — paraoxonase activity polymorphism, Humbert 1993 Nature Genetics), and <code>SOD3 R213G</code> (rs1799895 — disrupts heparin-binding domain, redistributes extracellular SOD from tissue matrix to plasma 10–30×, Bowler 2014). The bar for inclusion was strongly replicated functional effect — clinical disease-association literature for oxidative-stress SNPs is mixed and we frame these as "antioxidant defense capacity, context not prediction."',
      '<b>EMF assessments now surface your relevant variants.</b> When an assessment shows yellow / orange / red severity and you have DNA imported, a new "Oxidative-stress &amp; detox variants" panel appears alongside the room data.',
      '<b>Two of these surface in the Environment card Tips modal with actionable, non-supplement guidance:</b> NQO1 Ser/Ser flags benzene/smoke/exhaust avoidance; PON1 Q/Q flags slow organophosphate-pesticide hydrolysis (wash produce, organic for the EWG \'Dirty Dozen\', avoid garden-pesticide contact). No supplement push — just exposure context.',
      '<b>The "Interpret" button in the EMF assessment now factors in your genetics.</b> When DNA is imported, the AI receives your oxidative-stress variants alongside the room measurements and uses them to prioritize mitigations (e.g. separate sleeping areas from gas appliances if you carry NQO1 Ser/Ser). The system prompt explicitly forbids claiming any genotype causes or predicts EHS, and forbids supplement recommendations from genotype alone.',
      '<b>Honest framing:</b> there is no validated genetic test for electromagnetic hypersensitivity (EHS). All entries are flagged as mechanistic-only — the pathways are biologically engaged by EMF (Yakymenko 2016 meta-analysis) but no SNP we surface has been directly tested in EHS cohorts. The only direct EHS genetic study (De Luca 2014, n=153) found its 9.7× signal on <i>GSTM1+GSTT1 gene deletions</i>, which consumer DNA arrays don\'t reliably report, so we don\'t surface them.',
