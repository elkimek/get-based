...
const CHANGELOG = [
  {
    ...
    items: [
      '<b>Eight new SNPs and a new "Oxidative Stress" category in the genetics section:</b> <code>GSTP1 Ile105Val</code> (rs1695, glutathione conjugation), <code>SOD2 Ala16Val</code> (rs4880, mitochondrial superoxide dismutation), <code>CAT -262C/T</code> (rs1001179, catalase regulation), <code>NQO1 Pro187Ser</code> (rs1800566, quinone reduction), <code>GPX1 Pro198Leu</code> (rs1050450, glutathione peroxidase), <code>PON1 Q192R</code> (rs662, paraoxonase / LDL oxidation), <code>NFE2L2 -617C/A</code> (rs6721961, the Nrf2 antioxidant master-switch), and <code>SOD3 R213G</code> (rs1799895, extracellular SOD localization). Together these cover the major nodes of the body\'s oxidative-stress recovery system.',
      ...
      '<b>Honest framing:</b> there is no validated genetic test for electromagnetic hypersensitivity (EHS). All four entries are flagged as mechanistic-only — the pathways are biologically engaged by EMF (Yakymenko 2016 meta-analysis) but no SNP we surface has been directly tested in EHS cohorts. The only direct EHS genetic study (De Luca 2014, n=153) found its 9.7× signal on <i>GSTM1+GSTT1 gene deletions</i>, NOT on any of these SNPs — and consumer DNA arrays don\'t reliably report those deletions, so we don\'t surface them. Treat the variants here as general detox-capacity hints, not EMF-specific predictions.',
