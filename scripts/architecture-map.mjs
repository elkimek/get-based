...
/**
 * Return literal ESM dependencies without matching comments or string content.
 * Dynamic imports with computed specifiers are reported separately because
 * their targets cannot be checked statically.
 *
 * @param {string} source
...
function resolveRelativeImport(fromFile, specifier, moduleFiles) {
  ...
  const rawTarget = path.resolve(path.dirname(fromFile), cleanSpecifier);
...
function renderMap(architecture, rules) {
  ...
  const lines = [
    ...
    '| --- | --- | --- |',
    ...rules.groups.map(group => `| ${group.name} | ${group.roots.map(root => `\`${root}/\``).join(', ')} | ${group.mayImport.join(', ')} |`),
    '',
