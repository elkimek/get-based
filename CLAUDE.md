...
- **☀ Light**: sun sessions with Bird-Riordan spectral reconstruction, 24-preset photobiology device library, indoor light environment (rooms + screens), 8 on-device measurement tools, 6 channels (vit D / circadian / NIR / NO-cardiovascular / POMC / violet-eye), per-channel weekly chart, manual UVI override, photosensitive-medication awareness, burn-threshold alerts (70% / 100% MED), cumulative carry-over chip, altitude UV chip
...
- **`js/`** — ES modules loaded via `js/main.js` (count drifts; `ls js/*.js | wc -l`). Grouped by concern below; read the source for exported symbols.
  ...
  - **Sync + agent access**: `sync.js` (Evolu CRDT — Phase 1 dual-write across 37 importedData surfaces via 3 planners + Phase 2 cutover gate; debounced `onDataSaved`; header badge), `settings.js` (modal + sync setup), `data-merge.js` (composite-keyed conflict resolution + 200-cap on changeHistory, exports `COMPOSITE_KEYED_ARRAYS`)
  ...
  - **Light & Sun**: `sun.js` (sessions, live ticker, channel doses, hydrate, MED math, vitamin D IU, rolling totals), `sun-spectrum.js` (Bird-Riordan radiative transfer with proper Bass-Paur ozone cross-sections, 6 action spectra, vitaminDIU + fractionOfMED + erythemalSED + photosensitive scaling), `sun-uvdata.js` (Open-Meteo + selfhost CAMS + manual atmosphere, _isValidSelfhostUrl SSRF guard), `sun-defaults.js` (4-question Light setup card + 10-Q light-burden audit + photosensitive-meds toggle), `sun-context.js` (AI-context tier emitter, ~520 tok always-tier), `sun-correlations.js` (per-channel × biomarker Pearson, 12-week rolling, n≥4 overlapping weeks), `light-devices.js` (24-preset device library + custom devices + sessions), `light-env.js` (rooms + screens disclosure cards with chip pickers + light audits before/after compare), `light-tools.js` (8 measurement tools: Lux Meter / Flicker Detector / CCT Meter / Spectrum Classifier / Glass Transmission / Sleep Darkness / Sunrise Logger / Eye-Level Audit), `blob-storage.js` (IDB wrapper for >5MB importedData blobs).
...
2. *Per-row CRDT deltas* — the new path. Three planners (`_planArrayDelta`, `_planKeyedMapDelta`, `_planScalarDelta`) emit small per-item inserts/updates/tombstones into a generic `itemRow` table keyed by `arrayName + itemId`. Covers **37 importedData surfaces**: 10 arrays (`DELTA_ARRAYS` — sunSessions, lightDevices, deviceSessions, lightAudits, lightMeasurements, entries, notes, supplements, healthGoals, changeHistory), 7 keyed maps (`DELTA_MAPS` — markerNotes, customMarkers, manualValues, refOverrides, categoryLabels, categoryIcons, markerLabels), and 20 scalars (`DELTA_SCALARS` — context cards + menstrualCycle + genetics + biometrics + lightEnvironment + sunCorrelations + lifelightProfile + sunDefaults + emfAssessment + interpretiveLens + contextNotes + wearableSummary + wearableCardOrder).
