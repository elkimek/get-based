  ...
  async function refreshRoutes() {
    ...
    for (const connection of Array.isArray(registry?.connections) ? registry.connections : []) {
      ...
      for (const profile of profiles) {
        ...
        if (client && (client.baseUrl !== baseUrl || client.token !== token || status !== 'available')) {
          await client.close();
          clients.delete(id);
          client = null;
        }
    ...
    const retained = new Set(routes.filter(route => route.status === 'available').map(route => route.id));
