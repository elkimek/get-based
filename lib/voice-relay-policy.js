...
const PROVIDERS = new Set(['xai', 'elevenlabs', 'openrouter', 'ppq', 'venice']);
...
export function normalizeVoiceTtsPayload(provider, payload) {
  ...
  if (['openrouter', 'ppq', 'venice'].includes(provider)) {
    ...
    return {
      ...
      ...(provider === 'ppq' ? {} : { speed: rate }),
      ...(provider === 'ppq' ? {} : { response_format: 'mp3' }),
  ...
  if (['openrouter', 'ppq', 'venice'].includes(provider)) {
    if (action === 'voices') {
      ...
      throw createErrorWithCode(
        ...
        'This provider does not expose a compatible voice catalogue endpoint.',
      );
    }
    ...
    const headers = { Authorization: `Bearer ${key}` };
    if (action === 'stt') {
      return { url: `${baseUrl}/audio/transcriptions`, method: 'POST', headers };
    }
    return {
