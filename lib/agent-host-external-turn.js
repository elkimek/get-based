// @ts-check
// Turn normalization shared by ACP harnesses and Claude Code.
...
export function startExternalAgentTurn(options) {
  ...
  void (async () => {
    ...
    let mcpContext;
    try {
      const temporaryKey = `${options.agentId}:${sessionId || randomUUID()}`;
      ...
      mcpContext.session.tools = options.dynamicTools;
      const mcpEnvironment = { GETBASED_MCP_ENDPOINT: options.origin, GETBASED_MCP_TOKEN: mcpContext.token };
      const acpMcpServers = [{
      ...
      const textPrompt = `${!resumed ? `${instructionText}${historyText}\n\nUser request:\n` : ''}${options.prompt}${schemaText}`;
      ...
      if (options.agent.protocol === 'acp') {
        ...
        activeKey = `${options.agentId}:${sessionId}`;
        if (temporaryKey !== activeKey) {
          options.sessionMcp.delete(temporaryKey);
          options.sessionMcp.set(activeKey, mcpContext);
        }
        mcpContext.session.activeKey = activeKey;
        await options.agent.client.configureSession(sessionId, session.configOptions, options.model || '', options.effort || '');
        ...
        options.sessionMcp.delete(activeKey);
      }
    } catch (error) {
