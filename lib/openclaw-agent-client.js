...
export class OpenClawAgentClient {
  ...
  async readBoundedOutput(path) {
    ...
    if (info.size > MAX_OUTPUT_BYTES) throw new Error('OpenClaw response exceeded the companion limit.');
  ...
  async runCommand(args, signal) {
    ...
    try {
      ...
      // --config is authoritative for turns; the ambient path is included only
      // through the constrained overlay above.
  ...
  async prompt(options) {
    ...
    try {
      await Promise.all([
        writeFile(configPath, JSON.stringify(config), { mode: 0o600, flag: 'wx' }),
        writeFile(promptPath, completePrompt, { mode: 0o600, flag: 'wx' }),
      ]);
      const args = [
