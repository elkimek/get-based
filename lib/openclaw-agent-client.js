...
export class OpenClawAgentClient {
  ...
  async readBoundedOutput(path) {
    ...
    try {
      ...
      if (!info.isFile() || info.size > MAX_OUTPUT_BYTES) throw new Error('OpenClaw response exceeded the companion limit.');
      const buffer = Buffer.alloc(MAX_OUTPUT_BYTES + 1);
      const { bytesRead } = await handle.read(buffer, 0, buffer.length, 0);
      if (bytesRead > MAX_OUTPUT_BYTES) throw new Error('OpenClaw response exceeded the companion limit.');
