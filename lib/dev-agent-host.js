...
import { detectLocalAgents, publicAgentDescriptors } from './local-agent-registry.js';
...
const LOCAL_CLI_SPECS = [
  ...
  { id: 'claude', command: 'claude', env: 'GETBASED_CLAUDE_COMMAND', name: 'Claude Code', description: 'Anthropic official CLI', compatible: true },
  ...
  { id: 'grok', command: 'grok', env: 'GETBASED_GROK_COMMAND', name: 'Grok Build', description: 'xAI coding CLI', compatible: true },
];
...
function detectLocalClis(execImpl, env, platform = process.platform) {
  ...
  return LOCAL_CLI_SPECS.flatMap(spec => {
    const command = String(env[spec.env] || spec.command).trim();
    try {
