...
export class ACPAgentClient extends EventEmitter {
  ...
  async ensureSession(options) {
    ...
    if (requested && this.sessions.has(requested)) {
      return { sessionId: requested, configOptions: this.sessionCatalogs.get(requested) || [] };
    }
    if (requested) {
      ...
      if (method) {
        ...
        this.sessions.add(requested);
        this.sessionCatalogs.set(requested, result?.configOptions || []);
        return { sessionId: requested, configOptions: result?.configOptions || [] };
      }
    }
    ...
    return { sessionId, configOptions: result?.configOptions || [], raw: result };
  }

  /** @param {string} sessionId @param {any[]} configOptions @param {string} model @param {string} effort */
  async configureSession(sessionId, configOptions, model, effort) {
    const changes = [
      { value: model, option: configOptions.find(item => item?.category === 'model' || item?.id === 'model') },
      { value: effort, option: configOptions.find(item => item?.category === 'thought_level'
        || item?.id === 'thought_level' || item?.id === 'reasoning_effort') },
    ];
    for (const change of changes) {
      if (!change.value || !change.option?.id || change.option.currentValue === change.value) continue;
      const result = await this.request('session/set_config_option', {
        sessionId, configId: change.option.id, value: change.value,
      });
  ...
  async getModelCatalog() {
    if (!this.modelCatalogPromise) this.modelCatalogPromise = (async () => {
      ...
      const catalog = fromInitialize.length ? fromInitialize : normalizeACPModelCatalog(
        (await this.ensureSession({ mcpServers: [] })).raw || { configOptions: [] },
      );
