...
/** @param {string} threadId @param {string} token @param {string} [agent] */
function createThreadHandle(threadId, token, agent = 'codex') {
  ...
  return `v2.${payload}.${signThreadId(payload, token)}`;
}
...
function readThreadHandle(handle, token) {
  ...
  const current = handle.match(/^v2\.([a-z0-9-]{1,40})\.([A-Za-z0-9_-]{1,300})\.([A-Za-z0-9_-]{43})$/);
  if (!current || !tokenMatches(current[3], signThreadId(`${current[1]}.${current[2]}`, token))) return null;
  try {
...
export function createAgentHostService(options) {
  ...
  async function handleRequest(request) {
    ...
    if (url.pathname === '/v1/models' && request.method === 'GET') {
      ...
      try {
        ...
        if (agent.protocol === 'codex') {
          ...
          catalog = result?.data;
        } else catalog = await agent.client.getModelCatalog();
    ...
    const requestedSession = requestedThreadHandle ? readThreadHandle(requestedThreadHandle, token) : null;
    ...
    if (agent.protocol !== 'codex') {
      const abort = startExternalAgentTurn({
        ...
        createHandle: sessionId => createThreadHandle(sessionId, token, agentId),
      });
