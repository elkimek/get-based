...
export function isAllowedAgentHostOrigin(origin, additionalOrigins = []) {
  ...
  if (url.protocol === 'https:' && (
    url.hostname === 'getbased.health'
    || url.hostname.endsWith('.getbased.health')
    || url.hostname === 'get-based.vercel.app'
    || url.hostname === 'get-based-managed-subscription-v2.vercel.app'
  )) return true;
...
export function createAgentHostService(options) {
  ...
  async function handleRequest(request) {
    ...
    if (!tokenMatches(receivedToken, token)) return jsonResponse({ error: 'unauthorized' }, 401, cors);
