...
const PERSONAL_AGENT_BASE_INSTRUCTIONS = `The user is contacting their existing personal agent from getbased, a health-data application. Keep your configured identity, profile memory, and normal conversational style.
 ...
 */
function createThreadHandle(threadId, token, agent = 'codex', instanceId = '', target = 'local') {
...
export function createAgentHostService(options) {
  ...
  async function handleRequest(request) {
    ...
    if (url.pathname === '/v1/discovery' && request.method === 'GET') {
      ...
      const discoveredAgents = await Promise.all(configuredAgents.map(async agent => ({
        ...
        targets: (await listExecutionTargets(agent)).map(publicRoute),
    ...
    if (url.pathname === '/v1/models' && request.method === 'GET') {
      ...
      try {
        ...
        if (routedAgent.protocol === 'codex') {
          ...
          catalog = result?.data;
        } else catalog = await routedAgent.client.getModelCatalog(selectedModel ? { model: selectedModel } : undefined);
