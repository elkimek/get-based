...
export function renderWindowsCompanionRunner(options) {
  ...
  return `Set shell = CreateObject("WScript.Shell")\r\n${options.codexCommand ? `shell.Environment("Process")("GETBASED_CODEX_COMMAND") = ${vbsString(options.codexCommand)}\r\n` : ''}${options.sourceCodexHome ? `shell.Environment("Process")("GETBASED_SOURCE_CODEX_HOME") = ${vbsString(options.sourceCodexHome)}\r\n` : ''}${options.pathValue ? `shell.Environment("Process")("PATH") = ${vbsString(options.pathValue)}\r\n` : ''}shell.Run ${vbsString(command)}, 0, True\r\n`;
}
...
/**
 * @param {{bundlePath: string, env?: NodeJS.ProcessEnv, homeDirectory?: string, nodePath?: string, platform?: NodeJS.Platform, dryRun?: boolean, execFileSyncImpl?: typeof execFileSync}} options
 */
export function installWindowsCompanion(options) {
  ...
  const codexCommand = findWindowsExecutable(String(env.GETBASED_CODEX_COMMAND || 'codex').trim(), env);
  ...
  if (!existsSync(win32.join(sourceCodexHome, 'auth.json'))) throw new Error('Codex login was not found. Run `codex login` first.');
